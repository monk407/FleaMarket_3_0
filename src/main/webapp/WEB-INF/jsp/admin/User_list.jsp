...
<div id="admin">
    ...
    <div class="right">
        ...
        <div class="user_table">
            ...
            <c:forEach var="user" items="${userpage.list}" varStatus="statu">
               <c:if test="${user.id==1}">
                   <div class="show_user">
                       <ul>
                           <li>
                           ...
                           <li>
                               <div class="user_operation">
                                   ...
                                   <button type="button" class="btn_update" id="${user.id}" disabled="disabled">修改权限</button>
                                   <button type="button" class="btn_delete" id="${user.id}" disabled="disabled">删除</button>
                               </div>
                           </li>
                       </ul>
                   </div>
...
<br>
            <div style="text-align: center;">
                <c:if test="${userpage.hasPreviousPage}"><a href="/admin/AdmingetUserPage.do?pageNum=1">首页</a></c:if>
                <c:if test="${userpage.hasPreviousPage}"><a
                        href="/admin/AdmingetUserPage.do?pageNum=${userpage.prePage}">上一页</a></c:if>
                <c:forEach var="num" items="${userpage.navigatepageNums}">
                    <c:if test="${userpage.pageNum == num}">
                        <a style="color: red" href="/admin/AdmingetUserPage.do?pageNum=${num}">[${num}]</a>
                    </c:if>
                    <c:if test="${userpage.pageNum != num}">
                        <a href="/admin/AdmingetUserPage.do?pageNum=${num}">[${num}]</a>
                    </c:if>
                </c:forEach>
                <c:if test="${userpage.hasNextPage}"><a
                        href="/admin/AdmingetUserPage.do?pageNum=${userpage.nextPage}">下一页</a></c:if>
                <c:if test="${userpage.hasNextPage}"><a
                        href="/admin/AdmingetUserPage.do?pageNum=${userpage.pages}">尾页</a></c:if>

                <input style="width: 50px" type="text" name="goPage" id="goPage" class="input" value="1"
                       placeholder="页号">
